...
{
	if (IsKeyDown(KEY_A) && Timer - GetTime() < 100)
	{
		UpdateSelected(-1);
		Timer = GetTime();
	}
	if (IsKeyDown(KEY_D) && Timer - GetTime() < 100)
	{
		UpdateSelected(1);
		Timer = GetTime();
	}
	...
	//DrawRectangle(GetScreenWidth() / 2 - 100 + (Selected * 75), GetScreenHeight() / 2 - 10 - 50, 60, 25, YELLOW);
